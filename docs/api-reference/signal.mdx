...
outcome, because a bare confirmation cannot say _why_ a signal failed.
...
fan-out: a flood of depth _d_ over nodes of degree _f_ touches on the order of
_f^d_ synapses. Use it sparingly.
...
`Acknowledged` guarantees at-least-once delivery _or_ a receipt saying it
failed.
