...
description: "Normative rules for routing a signal through an NTL network: TTL, loop prevention, deduplication, signature verification and path selection."
...
**Deduplication and TTL MUST be checked _after_ verification.** 0.2.0-draft
required the opposite — verification after size, TTL _and_ dedup, to keep an
...
- **Deduplication is a write, not a read.** Claiming a signal identifier is
  ...
  _unauthenticated_ traffic occupy the dedup cache and suppress the genuine
  ...
  cap deliberately does not bound ([threat-model](/spec/threat-model) §8).
- **A TTL refusal owes an acknowledged sender a receipt**
  ([delivery-semantics](/spec/delivery-semantics) §2.2). Emitting one for a
...
to verify. See [threat-model](/spec/threat-model) §9 for the consequence: in
...
choice among _eligible_ paths, it does not override eligibility.
...
weights are _used_; that one specifies how they _change_.
